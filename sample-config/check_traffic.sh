...
#       "/var/tmp/check_traffic_${Host}_${Interface}.hist_dat_${USER}_64|32" for resolving 1)user and 2)64/32bit transfer problem which make 1)the hist data file read/write
#       error and 2)with huge error traffic value.
...
print_full_help_msg(){
	$Echo "Usage:"
	$Echo "$0 [ -v ] [ -6 ] [ -i Suffix ] [ -F s|S ] [-p N] [ -r ] -V 1|2c|3 -C snmp-community -H host [ -L ] -I interface -w in,out-warning-value  -c in,out-critical-value -K/M -B/b "
	$Echo "Example:"
...
if [ $# -lt 1 ]; then
	...
	while getopts :v6rhi:p:F:V:C:H:I:LKMBbw:c: OPTION
	do
		case $OPTION
			in
			v)
...
ifSpeed=`$SNMPWALK -v $Version -c $Community $Host IF-MIB::ifSpeed.${Interface}| awk -F ":" '{print $4}'`

$SNMPWALK -v $Version -c $Community $Host IF-MIB::ifHCOutOctets |grep Counter64 >/dev/null 2>&1 
Flag64=$?
