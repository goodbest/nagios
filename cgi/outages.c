...
void display_network_outages(void){
	...
	printf("<TR>\n");
	printf("<TH CLASS='data'>严重性</TH><TH CLASS='data'>主机</TH><TH CLASS='data'>状态</TH><TH CLASS='data'>备注</TH><TH CLASS='data'>持续时间</TH><TH CLASS='data'>受影响的主机数</TH><TH CLASS='data'>受影响的服务数</TH><TH CLASS='data'>动作</TH>\n");
	printf("</TR>\n");
	...
	for(temp_hostoutagesort=hostoutagesort_list;temp_hostoutagesort!=NULL;temp_hostoutagesort=temp_hostoutagesort->next){
		...
		snprintf(state_duration,sizeof(state_duration)-1,"%2d日 %2d时 %2d分 %2d秒",days,hours,minutes,seconds,(temp_hoststatus->last_state_change==(time_t)0)?"+":"");
