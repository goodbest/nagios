...
void compute_subject_availability_times(int first_state,int last_state,time_t real_start_time,time_t start_time,time_t end_time,avail_subject *subject, archived_state *as){
	...
	printf("%s 的 %lu 到 %lu (%lu 秒)计算 %d->%d 。<br>\n",(subject->type==HOST_SUBJECT)?"主机":"服务",start_time,end_time,(end_time-start_time),first_state,last_state);
#endif
...
void write_log_entries(avail_subject *subject){
	...
	printf("<DIV ALIGN=CENTER CLASS='dataTitle'>%s日志项:</DIV>\n",(subject->type==HOST_SUBJECT)?"主机":"服务");
...
void display_specific_servicegroup_availability(servicegroup *sg){
	...
	printf("<DIV ALIGN=CENTER>\n");
	printf("<TABLE BORDER=0 CLASS='data'>\n");
	printf("<TR><TH CLASS='data'>主机名</TH><TH CLASS='data'>服务</TH><TH CLASS='data'> 正常时间<BR>(OK)</TH><TH CLASS='data'> 告警时间<BR>(WARNING)</TH><TH CLASS='data'> 未知时间<BR>(UNKNOWN)</TH><TH CLASS='data'> 宕机时间<BR>(CRITICAL)</TH><TH CLASS='data'> 不可判断时间<BR>(UNDETERMINED)</TH></TR>\n");
...
void display_host_availability(void){
	...
	if(show_all_hosts==FALSE){
		...
		snprintf(time_indeterminate_nodata_string,sizeof(time_indeterminate_nodata_string)-1,"%d日 %d时 %d %d秒",days,hours,minutes,seconds);
		...
		printf("&t1=%lu&t2=%lu&includesoftstates=%s&assumestateretention=%s&assumeinitialstates=%s&assumestatesduringnotrunning=%s&initialassumedhoststate=%d&backtrack=%d' border=1 alt='主机状态趋势' title='Host State Trends' width='500' height='20'>",t1,t2,(include_soft_states==TRUE)?"yes":"no",(assume_state_retention==TRUE)?"yes":"no",(assume_initial_states==TRUE)?"yes":"no",(assume_states_during_notrunning==TRUE)?"yes":"no",initial_assumed_host_state,backtrack_archives);
		printf("</a><br>\n");
		printf("</p>\n");
#endif
		printf("<DIV ALIGN=CENTER>\n");
	...
	/* display data for all hosts */
		...
		if(output_format==HTML_OUTPUT){
			...
			printf("<DIV ALIGN=CENTER>\n");
			printf("<TABLE BORDER=0 CLASS='data'>\n");
			printf("<TR><TH CLASS='data'>主机</TH><TH CLASS='data'> 正常时间<BR>(OK)</TH><TH CLASS='data'> 宕机时间<BR>(DOWN)</TH><TH CLASS='data'> 不可达时间<BR>(UNREACHABLE)</TH><TH CLASS='data'> 不可判断时间<BR>(UNDETERMINED)</TH></TR>\n");
		        }
...
void display_service_availability(void){
	...
	if(show_all_services==FALSE){
		...
		printf("<img src='%s?createimage&smallimage&host=%s",TRENDS_CGI,url_encode(host_name));
		printf("&service=%s&t1=%lu&t2=%lu&includesoftstates=%s&assumestateretention=%s&assumeinitialstates=%s&assumestatesduringnotrunning=%s&initialassumedservicestate=%d&backtrack=%d' border=1 alt='服务状态的趋势' title='Service State Trends' width='500' height='20'>",url_encode(svc_description),t1,t2,(include_soft_states==TRUE)?"yes":"no",(assume_state_retention==TRUE)?"yes":"no",(assume_initial_states==TRUE)?"yes":"no",(assume_states_during_notrunning==TRUE)?"yes":"no",initial_assumed_service_state,backtrack_archives);
		printf("</a><br>\n");
		printf("</p>\n");
#endif
		...
		/*
		printf("<td CLASS='dataEven'>未按时间表</td><td CLASS='dataEven'>%s</td><td CLASS='dataEven'>%2.3f%%</td><td CLASS='dataEven'></td></tr>\n",time_indeterminate_unscheduled_string,percent_time_indeterminate_unscheduled);
		printf("<tr CLASS='dataEven'><td CLASS='dataEven'>按时间表</td><td CLASS='dataEven'>%s</td><td CLASS='dataEven'>%2.3f%%</td><td CLASS='dataEven'></td></tr>\n",time_indeterminate_scheduled_string,percent_time_indeterminate_scheduled);
		*/
	...
	/* display data for all services */
		...
		if(output_format==HTML_OUTPUT){
			...
			printf("<DIV ALIGN=CENTER>\n");
			printf("<TABLE BORDER=0 CLASS='data'>\n");
			printf("<TR><TH CLASS='data'>主机名</TH><TH CLASS='data'>服务</TH><TH CLASS='data'> 正常时间<BR>(OK)</TH><TH CLASS='data'> 告警时间<BR>(WARNING)</TH><TH CLASS='data'> 未知时间<BR>(UNKNOWN)</TH><TH CLASS='data'> 紧急时间<BR>(CRITICAL)</TH><TH CLASS='data'> 不确定时间<BR>(UNDETERMINED)</TH></TR>\n");
		        }
		else if(output_format==CSV_OUTPUT){
