...
void display_notifications(void){
	...
	if(use_lifo==TRUE){
		...
		if(result!=LIFO_OK){
			if(result==LIFO_ERROR_MEMORY){
				...
				printf("<P><DIV CLASS='errorMessage'>错误: 无法打开日志文件进行读。</DIV></P>",log_file_to_use);
				return;
			        }
			use_lifo=FALSE;
		        }
	...
	printf("<th CLASS='notifications'>状态</th>\n");
	printf("</tr>\n");
		...
		if(strstr(input,HOST_NOTIFICATION_STRING)||strstr(input,SERVICE_NOTIFICATION_STRING)){
			...
			if(notification_type==SERVICE_NOTIFICATION){
				...
				if(!strcmp(alert_level,"CRITICAL")){
					...
					strcpy(alert_level,"确认状态(ACKNOWLEDGEMENT)");
	...
	if(total_notifications==0){
		...
		printf("通知历史");
		printf("在%s的中",(log_archive==0)?"当前":"该档案");
		printf("无通知记录。</DIV></P>\n");
	        }
