...
int main(int argc, char **argv){
	...
	if(mode==CREATE_HTML && display_header==TRUE){
		...
		if(display_type==DISPLAY_HOST_HISTOGRAM)
			...
			snprintf(temp_buffer,sizeof(temp_buffer)-1,"主机ˇ服务的告警柱状图");
		...
		if(display_type!=DISPLAY_NO_HISTOGRAM && input_type==GET_INPUT_NONE){
			...
			printf("<tr><td CLASS='optBoxItem' valign=top align=left>事件到图:</td><td CLASS='optBoxItem' valign=top align=left>忽略重复状态:</td></tr>\n");
			printf("<tr><td CLASS='optBoxItem' valign=top align=left>\n");
	...
	if(display_type==DISPLAY_NO_HISTOGRAM || input_type!=GET_INPUT_NONE){
		...
		/* ask the user for report range and options */
			...
			printf("<tr><td valign=top calss='reportSelectSubTitle'>如果自定义报告周期...</td></tr>\n");
			...
			printf("<td class='reportSelectItem'>\n");
			printf("<select name='graphstatetypes'>\n");
			printf("<option value=%d>硬(Hard)\n",GRAPH_HARD_STATETYPES);
			printf("<option value=%d>软(Soft)\n",GRAPH_SOFT_STATETYPES);
			printf("<option value=%d SELECTED>硬(Hard)和软(Soft)\n",GRAPH_ALL_STATETYPES);
			printf("</select>\n");
			printf("</td></tr>\n");
...
//	char *months[12]={"January","February","March","April","May","June","July","August","September","October","November","December"};
	...
	*/
	...
	if( (strlen(ttf_file) > 5) && (access(ttf_file, F_OK) == 0) ){
		snprintf(temp_buffer,sizeof(temp_buffer)-1,"次数");
		temp_buffer[sizeof(temp_buffer)-1]='\x0';
		string_width=gdFontSmall->w*strlen(temp_buffer);
	...
	if( (strlen(ttf_file) > 5) && (access(ttf_file, F_OK) == 0) ){
		if(breakdown_type==BREAKDOWN_MONTHLY)
			snprintf(temp_buffer,sizeof(temp_buffer)-1," 每月 ");	//EDIT
		else if(breakdown_type==BREAKDOWN_DAY_OF_MONTH)
			snprintf(temp_buffer,sizeof(temp_buffer)-1," 每月按日 ");	//EDIT
		else if(breakdown_type==BREAKDOWN_DAY_OF_WEEK)
			snprintf(temp_buffer,sizeof(temp_buffer)-1," 每周按日 ");	//EDIT
		else
			snprintf(temp_buffer,sizeof(temp_buffer)-1," 每天按小时(15分钟间隔) ");	//EDIT
		temp_buffer[sizeof(temp_buffer)-1]='\x0';
		string_width=gdFontSmall->w*strlen(temp_buffer);
		if( (breakdown_type==BREAKDOWN_MONTHLY) || (breakdown_type==BREAKDOWN_DAY_OF_MONTH) || (breakdown_type==BREAKDOWN_DAY_OF_WEEK) ){
	...
	if( (strlen(ttf_file) > 5) && (access(ttf_file, F_OK) == 0) ){
		...
		if(display_type==DISPLAY_HOST_HISTOGRAM)
			snprintf(temp_buffer,sizeof(temp_buffer)-1,"主机 '%s' 的事件历史",host_name);
		else
			snprintf(temp_buffer,sizeof(temp_buffer)-1,"主机 '%s' 上的 '%s' 服务的事件历史",host_name,svc_description);
		temp_buffer[sizeof(temp_buffer)-1]='\x0';
		string_width=gdFontSmall->w*strlen(temp_buffer);
	...
	if( (strlen(ttf_file) > 5) && (access(ttf_file, F_OK) == 0) ){
		...
		snprintf(temp_buffer,sizeof(temp_buffer)-1,"恢复(%s):",(display_type==DISPLAY_SERVICE_HISTOGRAM)?"正常":"正常");
		temp_buffer[sizeof(temp_buffer)-1]='\x0';
		string_width=gdFontSmall->w*strlen(temp_buffer);
		...
		snprintf(temp_buffer,sizeof(temp_buffer)-1,"%s:",(display_type==DISPLAY_SERVICE_HISTOGRAM)?"告警":"宕机");
		temp_buffer[sizeof(temp_buffer)-1]='\x0';
		string_width=gdFontSmall->w*strlen(temp_buffer);
		...
		if(display_type==DISPLAY_SERVICE_HISTOGRAM){
			...
			snprintf(temp_buffer,sizeof(temp_buffer)-1,"紧急:");
			temp_buffer[sizeof(temp_buffer)-1]='\x0';
			string_width=gdFontSmall->w*strlen(temp_buffer);
